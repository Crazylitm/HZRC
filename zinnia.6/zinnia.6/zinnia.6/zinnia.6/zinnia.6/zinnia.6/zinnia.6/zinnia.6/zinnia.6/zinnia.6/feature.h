...
class Features {
 private:
  ...
  struct NodePair {
   ...
   std::vector<FeatureNodePair> featuresNodePair_;
  ...
  void AddNotePare(int sid,std::vector<NodePair> *node_pairs);//保存显著点
 public:
  bool read(const Character &character);
  const FeatureNode *get() const { return &features_[0]; }
    const FeatureNodePair *getPair() const { return &featuresNodePair_[0]; }
    size_t size() const { return featuresNodePair_.size(); }
  void NotePrint();
};
}
#endif
