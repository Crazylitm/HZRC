...
class ResultImpl: public Result {
 public:
  ...
  void add(ResultNote x,float y) {
    ...
    void add(const char *hz){
        results_hz.push_back(hz);
    }
    void clear() { results_.clear(); results_notepairs_.clear();results_hz.clear(); }
  const char *value(size_t i) const {
    return (i >= results_.size()) ? 0 : results_[i].second;
  }
    const char *hz_rec(size_t i)const{
        return (i >= results_hz.size()) ? 0: results_hz[i];
    }
  ...
  size_t size_pairs() const {return results_notepairs_.size();}
    size_t size_hz() const{return results_hz.size();}
 ...
 private:
  ...
  std::vector<std::pair<ResultNote, float> > results_notepairs_;
    std::vector<const char*> results_hz;
...
#define PT_SIZE 100000
    ...
    zinnia::CStrokeSlice myStroke;
    myStroke.SetRecognizeSliceRLDU(result);
    
    std::vector<std::string> result2;
    int rec = myStroke.GetRecognizeHZResult(result2);
    
    
    //ResultImpl *result_rec = new ResultImpl;
if(rec>0)
{
    for(int i=0;i<result2.size();i++)
    {
       // printf("RESULT_REC=%s\n",result2[i].c_str());
       // result_rec->add(result2[i].c_str(),0.0);
        result->add(result2[i].c_str());
    }
}   
