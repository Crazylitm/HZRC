...
#  define ZINNIA_DLL_EXTERN extern
  ...
  ZINNIA_DLL_EXTERN int recog(char *myOutString);
  ...
  ZINNIA_DLL_EXTERN const char *zinnia_result_value(zinnia_result_t *result, size_t i);
    ZINNIA_DLL_EXTERN const char *zinnia_result_hz(zinnia_result_t *result, size_t i);
    ZINNIA_DLL_EXTERN  float      zinnia_result_getx(zinnia_result_t *result, size_t i);
    ZINNIA_DLL_EXTERN  float      zinnia_result_gety(zinnia_result_t *result, size_t i);
    ZINNIA_DLL_EXTERN  int      zinnia_result_getStorkeNum(zinnia_result_t *result, size_t i);
  ...
  ZINNIA_DLL_EXTERN size_t      zinnia_result_size(zinnia_result_t *result);
    ZINNIA_DLL_EXTERN size_t      zinnia_result_size_hz(zinnia_result_t *result);
  ZINNIA_DLL_EXTERN size_t      zinnia_result_size_pairs(zinnia_result_t *result);
  ...
  ZINNIA_DLL_EXTERN zinnia_result_t *zinnia_recognizer_classify(zinnia_recognizer_t *recognizer,
                                                                ...
                                                                size_t nbest);
  ZINNIA_DLL_EXTERN zinnia_result_t *zinnia_recognizer_classify2(zinnia_recognizer_t *recognizer,const zinnia_character_t *character);
  /* Trainer */
...
class Result {
 public:
  virtual const char *value(size_t i) const = 0;
   virtual const char *hz_rec(size_t i) const = 0;
    virtual float getx(size_t i ) const =0;
    virtual float gety(size_t i)  const =0;
    virtual int getStorkeNum(size_t i)  const =0;
  virtual float score(size_t i) const = 0;
  virtual size_t size() const = 0;
    virtual size_t size_pairs() const =0;
    virtual size_t size_hz() const =0;
  virtual ~Result() {}
};
...
class Recognizer {
 public:
  ...
  virtual Result *classify(const Character &character) const = 0;
  virtual const char *what() = 0;
  virtual ~Recognizer() {}
#ifndef SWIG
#ifdef _WIN32
